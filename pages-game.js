(() => {
  ...
  const defaultState = () => ({
    version:1, name:"荒野队长", level:1, xp:0, stage:1, coins:1280, amber:120, bones:45, horns:10,
  ...
  function load(){
    try {
      ...
      return {...defaultState(),...raw,skills:raw.skills || defaultState().skills,order:raw.order || defaultState().order,pets:raw.pets || defaultState().pets,gear:raw.gear || defaultState().gear};
  ...
  const enemy = () => `${enemies[(state.stage-1)%enemies.length]}${state.stage%5===0?"·巨兽":""}`;
  ...
  const enemyPower = () => Math.floor(760*Math.pow(1.115,state.stage-1)*(state.stage%5===0?1.28:1));
  ...
  function home(){
    ...
    return `${heading(`第 ${state.stage} 关 · 长牙兽的足迹`,zone(),`<span class="chip">战力 ${fmt(power())}</span>`)}
  ...
  function battle(){
    const boss=state.stage%5===0,adv=power()>=enemyPower();
    const base=Math.floor((state.stage-1)/5)*5;
    return `${heading(`自动推图 · 第 ${state.stage} 关`,zone(),`<span class="chip">${adv?"战力优势":"需要养成"}</span>`)}
      <div class="battle-grid"><section class="arena card"><div class="arena-top"><span>我方 ${fmt(power())}</span><b>${enemy()}</b><span>敌方 ${fmt(enemyPower())}</span></div><div class="fighters"><div class="squad">${activePets().map(p=>petHtml(p)).join("")}</div><div class="vs">VS</div><div class="monster"><span>${boss?"兽":"野"}</span><b>${enemy()}</b></div></div><div class="battle-log">${state.lastLog}</div></section>
      ...
      <section class="route card"><h2>远征路线</h2><div class="nodes">${[1,2,3,4,5].map(n=>`<span class="${base+n===state.stage?"current":""}">${n===5?"兽":base+n}</span>`).join("")}</div></section>
      ${boss?`<section class="mechanics card"><h2>巨兽机制</h2><div><span class="done">破盾</span><span class="done">打断</span><span>净化</span><span>爆发</span></div></section>`:""}</aside></div>`;
  }
  function team(){
  ...
  function render(){
    ...
    if(tab==="battle"&&state.auto) battleTimer=setInterval(runBattle,state.speed===2?1700:3100);
  }
  function runBattle(){
    const ratio=power()/enemyPower(); const win=ratio>=1.25||Math.random()<Math.max(.08,Math.min(.96,.45+(ratio-1)*.7));
    if(!win){state.lastLog=`${enemy()} 冲散队形，小队撤回营火整备。`;save();render();return;}
    const boss=state.stage%5===0; const coins=Math.floor((48+state.stage*14)*(boss?2.8:1));
    ...
    state.lastLog=boss?"破甲骨矛击碎护盾，战吼打断蓄力！":`${enemy()} 已被击退，获得 ${fmt(coins)} 贝币。`;state.stage++;save();render();
  }
